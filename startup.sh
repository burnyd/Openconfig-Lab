...
docker create --name=influx -it -p 8083:8083 -p 8086:8086 -p 8088:8088 -p 8089:8089 -e INFLUXDB_DB=OC -e INFLUXDB_ADMIN_USER=dan -e INFLUXDB_ADMIN_PASSWORD=dan -e INFLUXDB_USER=dan -e INFLUXDB_USER_PASSWORD=dan -v $PWD/scripts/influxdb.conf:/etc/influxdb/influxdb.conf influxdb
...
curl -XPOST 'http://localhost:8086/query?u=myusername&p=mypassword' --data-urlencode 'q=CREATE DATABASE "opentsdb"'
#docker exec -it influx influx -execute 'CREATE DATABASE opentsdb'
