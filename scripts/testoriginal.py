...
def gen_request( xpath ):
    ...
    for path in xpath:
        mypath = path_from_string(path)
        #mysub = gnmi_pb2.Subscription(path=mypath, mode=opt.submode, suppress_redundant=opt.suppress, sample_interval=opt.interval*1000000000, heartbeat_interval=opt.heartbeat)
        mysub = gnmi_pb2.Subscription(path=mypath, mode=0, suppress_redundant=False, sample_interval=10*1000000000)
        mysubs.append(mysub)

    myprefix = path_from_string(None)
    mysblist = gnmi_pb2.SubscriptionList(prefix=myprefix, mode=0, allow_aggregation=True, encoding=4, subscription=mysubs)
    mysubreq = gnmi_pb2.SubscribeRequest( subscribe=mysblist )
...
print(gen_request(xpath))
#channel = grpc.insecure_channel('10.20.30.25:6030') #Need to change this simply to the target / node.
...
"""if __name__ == '__main__':
    ...
    if options.tls or options.cert:
        ...
        channel = grpc.insecure_channel(options.server) #Need to change this simply to the target / node.
        ...
        for response in responses:
            if response.HasField('sync_response'):
                ...
                if not options.stats:
                    ...
                    #oclog = log.info(response)
                    #print(response)
                    #producer.send('OC' , str(response).replace('update',''))
                    #struct =  json.dumps(response, indent=4)
                    #producer.send('OC' , struct)
            else:
