...
def _sub(stub, paths, username, password):
  if username:  # User/pass supplied for Authentication.
    mysblist = gnmi_pb2.SubscriptionList(prefix=[paths], mode='STREAM', allow_aggregation=False, encoding='JSON_IETF', subscription=[paths])
    return stub.Subscribe(
        gnmi_pb2.SubscribeRequest(path=[paths], encoding='JSON_IETF'),
        metadata=[('username', username), ('password', password)])

_sub(stub, paths, user, password)
print(_stub())
